...
with open(m3u_file, "w", encoding="utf-8") as f:
    ...
    for st in stations:
        ...
        # Direkter Stream-Proxy ohne KODIPROP / ohne inputstream.adaptive
        stream_url = f"http://127.0.0.1:8088/live/{sid}.m3u8|User-Agent=okhttp/4.9.0"

        f.write(f'#EXTINF:-1 tvg-id="{sid}" tvg-name="{name}" tvg-logo="{logo}" group-title="Waipu.tv",{name}\n')
        f.write(f'{stream_url}\n')

print(f"/home/pi/waipu.m3u8 fuer nativen Player neu geschrieben ({len(stations)} Sender)!")
